...
test.describe('@DoctorWorking tests', () => {
  ...
  totalSetup('@e2eFlow @ui User should be able to login and Finish work', async ({ page, loginSetup, caseSetup, baseURL }) => {
    if (!baseURL) {
  ...
  totalSetup('@NextCaseButton @ui User should not be able to proceed without selecting a condition', async ({ page, loginSetup, caseSetup, baseURL }) => {
    if (!baseURL) {
      throw new Error("BASE_URL environment variable is not set")
    }
    const mainPage: MainPage = new MainPage(page);
